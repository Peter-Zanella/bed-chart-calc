...
def compute_yogas(planets: Dict, lagna_idx: int) -> List[Dict]:
    ...
    if len(adhi) >= 2:
        add("Adhi Yoga", "Moon", adhi, "Benefics in the 6th/7th/8th from the Moon.")
...
def compute_muhurta(activity: str, lat: float, lon: float, tz_offset: float,
                    start, days: int = 183) -> List[Dict]:
    """Scan `days` days from `start`, returning days whose Moon-nakshatra suits the
    activity, rated by vara/tithi/yoga/karana. Panchanga is sampled at local noon —
    a daily approximation; exact muhurta needs intraday timing."""
    ...
    for i in range(days):
        ...
        jd = get_jd(d.year, d.month, d.day, 12.0 - tz_offset)     # local noon
        sun, moon = _sun_moon_sid(jd)
        nak, nl, _ = nakshatra_of(moon)
        if nak not in good_nak:
            continue
        pan = compute_panchang(sun, moon, d.isoweekday() % 7, nak, nl)
        score, flags = 2, []
        if pan["vara"] in good_vara:
            score += 1
        else:
        ...
        out.append({"date": d.isoformat(), "weekday": pan["vara"], "nakshatra": nak,
                    "tithi": pan["tithi"], "yoga": pan["yoga"], "karana": pan["karana"],
