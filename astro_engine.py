...
def compute_muhurta(activity: str, lat: float, lon: float, tz_offset: float,
    ...
    """For each day, find the local-time WINDOW(S) in which the activity's favourable
    Moon-nakshatra is active (with interpolated boundary times), rated by vara/tithi/
    yoga/karana. Windowing avoids the skip/repeat artdefacts of a single daily sample."""
    ...
    for i in range(days):
        ...
        # collect favourable-nakshatra segments with interpolated start/end minutes
        segs = []
        cur = int(lons[0] / span)
        seg_start = 0.0
        for h in range(1, 25):
        ...
        for nak, a, b in segs:
            ...
            out.append({"date": d.isoformat(), "weekday": vara, "nakshatra": nak,
                        "window": f"{hm(a)}–{hm(b)}", "tithi": pan["tithi"],
