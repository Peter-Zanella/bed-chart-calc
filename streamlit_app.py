...
# ──────────────────────────────────────────────────────────────────────────────
# BHAVA (HOUSE) CHART — South-Indian box, SIGNS FIXED with Pisces at top-right;
# houses are shown as labels (counted from the Lagna). Planets placed by sign.
# ──────────────────────────────────────────────────────────────────────────────
# sign_idx -> (row, col) so that Pisces (11) sits top-right and the zodiac runs
# clockwise:  Sag Cap Aqu Pis / Sco . . Ari / Lib . . Tau / Vir Leo Can Gem
...
def bhava_grid_html(houses, occupants, planet_data, lagna_si) -> str:
    natal = {i: [] for i in range(12)}
    for p, pd in planet_data.items():
        if p != "Ascendant":
            natal[pd["sign_idx"]].append(p)
    grid = {rc: si for si, rc in _BHAVA_CELL.items()}
...
def build_pdf(chart: dict) -> bytes:
    ...
    story.append(KeepTogether(crow(
        si_table("Bhava (Houses)", d1_pl, chart["lagna_idx"], cellmap=_BHAVA_CELL),
        si_table(f"Transits {chart.get('transit_date','')}", trans_pl, chart["lagna_idx"]))))
    story.append(Paragraph("Bhava: signs fixed with Pisces top-right; boxes labelled by house "
                           "from the Lagna. Transits: current sky against the natal Lagna.", cap))
...
with tabs[0]:
    ...
    st.divider()
    st.subheader("Bhava chart — South Indian (Pisces top-right)")
    st.caption("Signs in fixed positions (Pisces top-right, zodiac clockwise), each box "
               "labelled with its house counted from the Lagna (◆). Planets sit in their sign.")
