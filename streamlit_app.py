...
# ──────────────────────────────────────────────────────────────────────────────
# BHAVA (HOUSE) CHART — standard South-Indian box (Pisces top-left, like the Rasi);
...
def build_pdf(chart: dict) -> bytes:
    ...
    story.append(KeepTogether(crow(
        si_table("Bhava (Houses)", d1_pl, chart["lagna_idx"]),
        si_table(f"Transits {chart.get('transit_date','')}", trans_pl, chart["lagna_idx"]))))
    story.append(Paragraph("Bhava: standard South-Indian layout (Pisces top-left); boxes "
                           "labelled by house from the Lagna. Transits: current sky vs natal Lagna.", cap))
...
with tabs[0]:
    ...
    st.divider()
    st.subheader("Bhava chart — South Indian (houses labelled)")
    st.caption("Standard South-Indian layout (Pisces top-left), each box labelled with its "
               "house counted from the Lagna (◆). Planets sit in their sign.")
