...
# ──────────────────────────────────────────────────────────────────────────────
# BHAVA (HOUSE) GRID — houses 1..12 in reading order, house as the organising key
# ──────────────────────────────────────────────────────────────────────────────
def bhava_grid_html(houses, occupants, planet_data, lagna_si) -> str:
    cells = []
    for h in range(1, 13):
        sn = houses[h]
        lord = E.SIGN_LORDS[sn]
        tag = " ◆La" if h == 1 else ""
        if occupants[h]:
            occ = "".join(
                f"<div class='bh-p'>{PLANET_ABR.get(p, p[:2])} "
                f"<span class='bh-deg'>{planet_data.get(p, {}).get('pos', '')}</span></div>"
                for p in occupants[h])
        else:
            occ = "<div class='bh-empty'>—</div>"
        cells.append(
            f"<td class='{'bh-l' if h == 1 else ''}'>"
            f"<div class='bh-hdr'>H{h}{tag}</div>"
            f"<div class='bh-sign'>{sn} · {lord}</div>{occ}</td>")
    rows = "".join("<tr>" + "".join(cells[r * 4:(r + 1) * 4]) + "</tr>" for r in range(3))
    return f"<div class='bh-wrap'><table class='bh'>{rows}</table></div>"
...
with tabs[0]:
    ...
    st.divider()
    st.subheader("Bhava chart — by house (◆ = Lagna / 1st house)")
    st.caption("Houses 1–12 in order (whole-sign), each with its sign, sign-lord and the "
               "planets sitting in it — a house-centric view, distinct from the sign-based Rasi.")
