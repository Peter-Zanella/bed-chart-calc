...
if mode == "Search by city":
    ...
    # a stored lookup is only valid for this exact query (offset depends on the date too)
    ...
    if geo and geo.get("_query") != cur_q:          # city or date/time changed → stale
        st.sidebar.info("City or date changed — press 🔍 Look up location to refresh.")
        geo = None
    if geo is None:
        st.sidebar.caption("Enter a city, then press 🔍 Look up location.")
