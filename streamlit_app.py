...
# ──────────────────────────────────────────────────────────────────────────────
# BHAVA (HOUSE) CHART — South-Indian-style box; houses 1..12 fixed clockwise
# ──────────────────────────────────────────────────────────────────────────────
# perimeter cells of a 4x4 grid, clockwise from top-left
_BHAVA_ORDER = [(0, 0), (0, 1), (0, 2), (0, 3), (1, 3), (2, 3),
                (3, 3), (3, 2), (3, 1), (3, 0), (2, 0), (1, 0)]


def bhava_grid_html(houses, occupants, planet_data, lagna_si) -> str:
    pos = {rc: i + 1 for i, rc in enumerate(_BHAVA_ORDER)}

    def cell(r, c):
        h = pos[(r, c)]
        sn = houses[h]
        is_l = (h == 1)
        hdr = f"{'◆ ' if is_l else ''}H{h} · {SIGN_ABR[E.SIGNS.index(sn)]}"
        body = "<br>".join(
            f"<span class='si-natal'>{PLANET_ABR.get(p, p[:2])} "
            f"<span class='bh-deg'>{planet_data.get(p, {}).get('pos','')}</span></span>"
            for p in occupants[h])
        return f"<td class='{'lagna' if is_l else ''}'><span class='si-hdr'>{hdr}</span>{body}</td>"

    rows = [
        "<tr>" + "".join(cell(0, c) for c in range(4)) + "</tr>",
        f"<tr>{cell(1,0)}"
        f"<td class='center' colspan='2' rowspan='2'>Bhava<br>(Houses)</td>"
        f"{cell(1,3)}</tr>",
...
def build_pdf(chart: dict) -> bytes:
    ...
    # ── bhava (house) chart + transit chart ─────────────────────────────────────
    def bhava_table(houses, occupants):
        order = [(0, 0), (0, 1), (0, 2), (0, 3), (1, 3), (2, 3),
                 (3, 3), (3, 2), (3, 1), (3, 0), (2, 0), (1, 0)]
        pos = {rc: i + 1 for i, rc in enumerate(order)}
        data = [[None] * 4 for _ in range(4)]
        for (r, c), h in pos.items():
            sn = houses[h]; tag = "◆" if h == 1 else ""
            txt = f"<b>H{h}{tag} {SIGN_ABR[SIGNS.index(sn)]}</b>"
            occ = [PLANET_ABR.get(p, p[:2]) for p in occupants[h]]
            if occ:
                txt += "<br/>" + " ".join(occ)
            data[r][c] = Paragraph(txt, small)
        data[1][1] = Paragraph("<b>Bhava</b>", ParagraphStyle(
            "bh", parent=small, alignment=1, textColor=accent, fontName="Times-Bold", fontSize=9))
        data[1][2] = ""; data[2][1] = ""; data[2][2] = ""
        t = Table(data, colWidths=[22 * mm] * 4, rowHeights=[18 * mm] * 4)
        styl = [("GRID", (0, 0), (-1, -1), 0.4, line), ("VALIGN", (0, 0), (-1, -1), "TOP"),
                ("BACKGROUND", (1, 1), (2, 2), paper), ("SPAN", (1, 1), (2, 2)),
                ("BACKGROUND", (0, 0), (0, 0), lagna_bg)]
        t.setStyle(TableStyle(styl))
        return t

    tr = chart.get("transits", {})
    trans_pl = {p: tr[p]["sign_idx"] for p in tr if p != "Ascendant"} if tr else {}
    story.append(Paragraph("Bhava (House) &amp; Transit Charts (South Indian)", h2))
    story.append(KeepTogether(crow(
        bhava_table(chart["houses"], chart["occupants"]),
        si_table(f"Transits {chart.get('transit_date','')}", trans_pl, chart["lagna_idx"]))))
    story.append(Paragraph("Bhava: houses 1–12 fixed clockwise from the Lagna. "
                           "Transits: current sky against the natal Lagna.", cap))
...
with tabs[0]:
    ...
    st.divider()
    st.subheader("Bhava chart — by house (South Indian style)")
    st.caption("Houses 1–12 fixed clockwise from the Lagna (◆ = 1st house), each box showing "
               "its sign and the planets in it — a house-centric view in the South-Indian format.")
    st.markdown(bhava_grid_html(chart["houses"], chart["occupants"],
                                chart["planets"], chart["lagna_idx"]), unsafe_allow_html=True)
...
with tabs[6]:
    ...
    st.caption("Current sky against the natal Lagna. House = counted from the natal "
               "Ascendant (whole-sign). Bindus = Ashtakavarga points on the transited sign.")
    ...
    with tcol2:
        ...
        st.dataframe([{"Planet": p, "Transit sign": SIGNS[tr[p]["sign_idx"]],
                       "Pos": tr[p]["pos"],
                       "House": (tr[p]["sign_idx"] - lagna) % 12 + 1,
